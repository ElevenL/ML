...
if __name__ == "__main__":
    ...
    for i in range(1000):
        filename = str(i) + '.gif'
        code_img = create_validate_code(size=(70, 100),
                             chars=init_chars,
                             img_type="GIF",
                             mode="RGB",
                             bg_color=(255, 255, 255),
                             ...
                             draw_points=True,
                             point_chance=3)
        code_img[0].save("./vcode/" + filename, "GIF")
        # print code_img[1]
        p = pd.DataFrame([[filename, code_img[1]]], columns=['filename', 'code'])
        recode = pd.concat([recode, p], ignore_index=True)
    recode.to_csv('./vcode/recode.csv')